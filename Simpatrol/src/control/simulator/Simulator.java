...
public abstract class Simulator {
	 ...
	 *            The time rate to atualize the internal model of the
	 *            simulation.
	 * @throws IOException
	 */
	...
	public PerceptionDaemon getPerceptionDaemon(Agent agent) {
		...
		for (int i = 0; i < perception_daemons_array.length; i++)
			if (((PerceptionDaemon) perception_daemons_array[i]).getAgent()
					.equals(agent))
	...
	public ActionDaemon getActionDaemon(Agent agent) {
		...
		for (int i = 0; i < action_daemons_array.length; i++)
			if (((ActionDaemon) action_daemons_array[i]).getAgent().equals(
					agent))
	...
	/**
	 * Returns the time rate, in seconds, to atualize the internal model of the
	 * simulation.
	 * 
	...
	public Mortal[] getMortalObjects() {
		...
		// obtains the mortal objects from the societies (mortal agents) of the
		// environment
		// for each society
