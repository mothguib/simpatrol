...
public abstract class CompoundActionsParser {
	 ...
	 * @param agent
	 *            The agent intendig to execute the action.
	 * @param path
			...
			if (remained_length > current_displacement) {
				...
				while (current_displacement >= remained_length) {
					...
					if (edges_current_path_vertex.length <= 1
							|| depth_limitation == 0) {
						Visible[] visible_objects_array = new Visible[visible_objects
								.size()];
						...
						teleport_actions.add(new TeleportAction(
								current_graph_vertex, null, 0,
								visible_objects_array));

						TeleportAction[] answer = new TeleportAction[teleport_actions
								.size()];
	...
	public static AtomicRechargeAction[] parseRechargeAction(
		...
		AtomicRechargeAction[] answer = new AtomicRechargeAction[atomic_actions
				.size()];
		for (int i = 0; i < answer.length; i++)
