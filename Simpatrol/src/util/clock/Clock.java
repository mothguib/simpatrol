...
/**
 * Implements a real time clock, that with a specific periodicity, sends signal
 * to a Clockable object in order to make it act.
 * 
 * @see Clockable
 */
public final class Clock extends Thread {
	...
	/**
	 * The clock count step. The default value is one second.
	 */
	private int step = 1;

	/**
	 * The clock count unity, as in Calendar field. The default value is
	 * Calendar.SECOND.
	 * 
	 * @see Calendar
	 */
	...
	/**
	 * The time interval used by the clock to let the clockable object act. Its
	...
	public void run() {
		while (!this.stop_working) {
			try {
				...
				int time_gap = (int) ((next_ref - prev_ref) * Math.pow(
						this.step, -1));
