...
public final class Coordinator extends Thread {
	...
	public void run() {
		...
		for (int i = 0; i < this.NUMBER_OF_CYCLES; i++) {
			...
			//System.err.println("Before perceived =" + per);			
			this.makeAgentsPerceive();
			long act = System.currentTimeMillis();
			perAvg += act - per;
			//System.err.println("Interval Perceived"+ (act -per));
			this.makeAgentsAct();
			long up = System.currentTimeMillis();
			actAvg += up - act;
			//System.err.println("Interval Act"+ (up - act));			
			this.updateEnvironmentModel();
			long total = System.currentTimeMillis();
			upAvg += total - up;
			//System.err.println("Interval Update"+ (total - up));
			//System.err.println("Cycle Time"+ (total - per));
		...
		System.err.println("Time of Simulation = "+((System.currentTimeMillis()-start_time)/1000)+ " seconds");
		System.err.println("Average Perception="+(perAvg/NUMBER_OF_CYCLES));
		System.err.println("Average Action="+(actAvg/NUMBER_OF_CYCLES));
		System.err.println("Average Update Enviroment="+(upAvg/NUMBER_OF_CYCLES));
