...
public class SimulationConfigurationGUI extends javax.swing.JDialog {
	...
	private void initWindow() {
		...
		this.simulation_mode_panel
				.setBorder(new TitledBorder("Simulation mode"));
		...
		this.atualization_rate_panel.setBorder(new TitledBorder(
				"Atualization rate"));
		this.atualization_rate_panel.add(this.atualization_rate_input,
				BorderLayout.CENTER);
	...
	private void ok_buttonActionPerformed(ActionEvent evt) {
		...
		double atualization_time_rate = ((Double) this.atualization_rate_input
				.getValue()).doubleValue();
		if (atualization_time_rate <= 0) {
			JOptionPane.showMessageDialog(this,
					"The atualization rate must be a real positive number.",
					"Atualization rate error", JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		((SimPatrolGUI) this.getParent()).configureSimulation(
				this.real_time_choice.isSelected(), ((Integer) this.port_input
						.getValue()).intValue(), atualization_time_rate);
