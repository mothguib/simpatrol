...
public class SimulationConfigurationGUI extends javax.swing.JDialog {
	...
	private JPanel configuration_panel;
		...
		private JPanel atualization_rate_panel;
			private JFormattedTextField atualization_rate_input;
	...
	/** Constructor.
	 * 
	 *  @param parent The SimPatrol's GUI. */
	...
	private void initWindow() {
        ...
        this.simulation_mode_panel.setBorder(new TitledBorder("Simulation mode"));
        this.real_time_choice.setSelected(true);
        ...
        this.port_panel.setBorder(new TitledBorder("Port number"));
        ...
        this.atualization_rate_panel.add(this.atualization_rate_input, BorderLayout.CENTER);
        this.sec_label.setText(" sec");
        this.atualization_rate_panel.add(this.sec_label, BorderLayout.EAST);
        this.configuration_panel.add(this.atualization_rate_panel);
        
        // 2.4. adds the configuration panel to the dialog window
        this.getContentPane().add(this.configuration_panel, BorderLayout.CENTER);
        ...
        setBounds((screenSize.width-350)/2, (screenSize.height-207)/2, 350, 207);
    }
    ...
    private void exitWindow(WindowEvent evt) {
    	...
    	if(!this.button_ok_clicked)
    		new SimulationConfigurationGUI((SimPatrolGUI) this.getParent()).setVisible(true);
    }
	
	/** Executed when the ok button is pressed. */
    private void ok_buttonActionPerformed(ActionEvent evt) {
    	// treats the atualization time rate
    	double atualization_time_rate = ((Double) this.atualization_rate_input.getValue()).doubleValue();
    	if(atualization_time_rate <= 0) {
    		JOptionPane.showMessageDialog(this, "The atualization rate must be a real positive number.", "Atualization rate error", JOptionPane.ERROR_MESSAGE);
    		return;
    	}
    	...
    	((SimPatrolGUI) this.getParent()).configureSimulation(this.real_time_choice.isSelected(), ((Integer) this.port_input.getValue()).intValue(), atualization_time_rate);
