	...
	private static Vertex[] getVertexes(Element xml_element) {
		...
		for(int i = 0; i < answer.length; i++) {
			...
			if(etpds.length == 0) {
				...
				if(str_is_enabled.length() > 0) is_enabled = Boolean.parseBoolean(str_is_enabled);
				
				// new dynamic vertex
				current_vertex = new DynamicVertex(label, etpds[0], etpds[1], is_enabled);
			}
	...
	private static Edge[] getEdges(Element xml_element, Vertex[] vertexes) {
		...
		for(int i = 0; i < answer.length; i++) {
			...
			if(etpds.length == 0) {
				...
				// new dynamic edge
				current_edge = new DynamicEdge(emitter, collector, oriented, length, etpds[0], etpds[1], is_enabled);
			}
			...
			// decides if the edge is in the emitter and collector enabling memories
