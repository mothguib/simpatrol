...
/** Implements a translator that obtains Metric objects
...
public abstract class MetricTranslator extends Translator {
	 ...
	 * 
	 *  @param xml_element The XML source containing the metric objects.
	...
	public static Metric[] getMetrics(Element xml_element) {
		...
		for(int i = 0; i < metric_node.getLength(); i++) {
			...
			switch(type) {
				...
				case(MetricTypes.MEAN_IDLENESS): {
					...
					if(str_value.length() > 0) initial_value = Double.parseDouble(str_value);
		...
		Metric[] answer = new Metric[metrics.size()];
		for(int i = 0; i < answer.length; i++)
			answer[i] = metrics.get(i);
		return answer;
	}
}