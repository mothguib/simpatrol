...
/** Implements a translator that obtains Agent objects
...
public abstract class AgentTranslator extends Translator {
	...
	private static PerceptionPermission[] getAllowedPerceptions(Element xml_element) {
		...
		for(int i = 0; i < answer.length; i++) {
			...
			PerceptionPermission allowed_perception =  new PerceptionPermission(LimitationTranslator.getLimitations(allowed_perception_element), perception_type);
	...
	private static ActionPermission[] getAllowedActions(Element xml_element) {
		...
		for(int i = 0; i < answer.length; i++) {
			...
			ActionPermission allowed_action =  new ActionPermission(LimitationTranslator.getLimitations(allowed_action_element), action_type);
