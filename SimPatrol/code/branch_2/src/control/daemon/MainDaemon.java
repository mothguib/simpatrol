...
public final class MainDaemon extends Daemon {
	...
	private void attendEnvironmentCreationConfiguration(
			EnvironmentCreationConfiguration configuration) throws IOException {
	...
	private void attendAgentDeathConfiguration(
			AgentDeathConfiguration configuration) throws IOException {
	 ...
	 *            The duration of a cycle of measurement of the given metric.
	...
	public void run() {
		while (!this.stop_working) {
			...
			while (message == null) {
				...
				// was eventually terminated (due to an error in the client
				// side)
				synchronized (simulator) {
					simulator.getState(); // synchronization...
