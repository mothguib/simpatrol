...
/** Parses the compound actions of the agents of SimPatrol
 *  into time chained atomic actions.
 *
 *  @see CompoundAction
 *  @developer New CompoundAction classes must change this one. */
public abstract class CompoundActionsParser {
	/** Parses the GoToAction objects into time
	 *  chained TeleportAction objects.
	 *  
	 *  @param action The action to be parsed.
	 *  @param agent The agent intendig to execute the action.
	 *  @param path The path the agent shall trespass on the Graph. 
	 *  @param depth_limitation The maximum number of edges the path shall be trespassed.
	 *  @param speed_limitation The maximum speed that the agent can reach when moving. Measured in depth/sec or depth/cycle.
	 *  @param acceleration_limitation The maximum acceleration that the agent can assume when moving. Measured in depth/sec^2 or depth/cycle^2.
	 *  @param time_rate The time rate used to generate the time chain of atomic actions. Measured in seconds, or in cycles.
	 *  @return The correspondent time chained actions of teleport. */
	public static TeleportAction[] parseGoToAction(GoToAction action, Agent agent, Graph path, int depth_limitation, double speed_limitation, double acceleration_limitation, double time_rate) {
		...
		if(acceleration_limitation > -1 && acceleration > acceleration_limitation)
		...
		while(true) {
			...
			// d = v0 * t + a * t^2 / 2 
			current_displacement = current_initial_speed * time_rate + acceleration * Math.pow(time_rate, 2) * 0.5;
			...
			// atualizes the current initial speed
			// v = v0 + a*t
			current_initial_speed = current_initial_speed + acceleration * time_rate;
			if(speed_limitation > -1 && current_initial_speed > speed_limitation)
				current_initial_speed = speed_limitation;			
		}
	}
	
	/** Parses the RechargeAction objects into time
	 *  chained AtomicRechargeAction objects.
	 *  
	 *  @param action The action to be parsed.
	 *  @param speed_limitation The maximum speed that the agent can recharge its stamina. Measured in stamina/sec or stamina/cycle.
	 *  @param time_rate The time rate used to generate the time chain of atomic actions. Measured in seconds, or in cycles.
	 *  @return The correspondent time chained actions of recharging. */
	public static AtomicRechargeAction[] parseRechargeAction(RechargeAction action, double speed_limitation, double time_rate) {
		...
		// mounts the answer for the method
		AtomicRechargeAction[] answer = new AtomicRechargeAction[atomic_actions.size()];
		for(int i = 0; i < answer.length; i++)
			answer[i] = atomic_actions.get(i);
