...
/** Implements a coordinator that assures the correct elapsing
 *  of cycles when the patrolling task is being simulated.
 *  
...
public final class Coordinator extends Thread implements TimedObject {
	...
	/** The set of trios formed by the agents and their respective
	 *  spent staminas due to their actions and perceptions. */
	private Set<AgentAndSpentStaminas> staminas;
	
	/* Methods. */
	/** Constructor.
	 * 
	 * @param cycled_simulator The simulator of the patrolling task. */
	 ...
	 *  the given agent.
	 *  
	 *  @param agent The agent whose stamina is being configured.
	...
	/** Returns the amount of stamina to be spent due to the actions of
	 *  the given agent.
	 *  
	 *  @param agent The agent whose stamina is wanted.
	 *  @return The amount of stamina to be spent due to the actions of the given agent. */
	...
	/** Configures the given amount of stamina to be spent due to the perceptions of
	 *  the given agent.
	 *  
	 *  @param agent The agent whose stamina is being configured.
	...
	private void assureMortality() throws IOException {
		...
		for(int i = 0; i < mortal_objects.length; i++) {
			...
			EventTimeProbabilityDistribution death_tpd = current_object.getDeathTPD();
			...
			if(this.staminas != null) {
				...
				for(int j = 0; j < staminas_array.length; j++) {
					...
					if(agents_staminas.AGENT.equals(agents[i])) {
						spent_stamina = agents_staminas.actions_spent_stamina + agents_staminas.perceptions_spent_stamina;
						break;
					}
				}
			}
	...
	public void run() {
		...
		for(int i = 0; i < this.NUMBER_OF_CYCLES; i++) {
			...
			// screen message
			System.out.println("[SimPatrol.Coordinator]: Agents are thinking.");
			
			// agent must act now
			// screen message
			System.out.println("[SimPatrol.Coordinator]: Agents are acting.");
...
/** Internal class that holds together an agent
 *  and the stamina values to be spent due to
 *  the perceptions and to the actions of such agent,
 *  respectively.  */
final class AgentAndSpentStaminas {
	...
	/** Constructor.
	 * 
	 *  @param agent The agent whose stamina is being controlled. */
