 ...
 *  that can become disabled or enabled with a specific event time
...
public final class DynamicEdge extends Edge implements Dynamic {
	 ...
	 *  @param enabling_tpd The time probability distribution for the edge enabling.
	 *  @param disabling_tpd The time probability distribution for the edge disabling.
	 *  @param is_enabled TRUE, if the edge is enabled, FALSE if not. */
	public DynamicEdge(Vertex vertex_1, Vertex vertex_2, double length, EventTimeProbabilityDistribution enabling_tpd, EventTimeProbabilityDistribution disabling_tpd, boolean is_enabled) {
		this(vertex_1, vertex_2, false, length, enabling_tpd, disabling_tpd, is_enabled);
	}
	 ...
	 *  @param disabling_tpd The time probability distribution for the edge disabling.
	 *  @param is_enabled TRUE, if the edge is enabled, FALSE if not. */
	public DynamicEdge(Vertex emitter, Vertex collector, boolean oriented, double length, EventTimeProbabilityDistribution enabling_tpd, EventTimeProbabilityDistribution disabling_tpd, boolean is_enabled) {
	 ...
	 *  @param disabling_tpd The time probability distribution for the edge disabling.
	 *  @param is_enabled TRUE, if the edge is enabled, FALSE if not.
	 *  @param id The id forced to the object. */	
	protected DynamicEdge(Vertex emitter, Vertex collector, boolean oriented, double length, EventTimeProbabilityDistribution enabling_tpd, EventTimeProbabilityDistribution disabling_tpd, boolean is_enabled, String id) {		
	...
	public DynamicEdge getCopy(Vertex copy_emitter, Vertex copy_collector) {
		...
		// the copy		
		DynamicEdge answer = new DynamicEdge(copy_emitter, copy_collector, oriented, this.length, this.enabling_tpd, this.disabling_tpd, this.is_enabled, id);
