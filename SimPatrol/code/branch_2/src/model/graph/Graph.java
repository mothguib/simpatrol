...
public final class Graph implements XMLable {
	 ...
	 *  depth is reached.
	 *  
	 *  Only the enabled and visible elements (vertexes and edges) are
	 *  added to the subgraph. 
	 *  
	 *  If the given depth is set to -1, the entire visible and enabled
	...
	public synchronized Graph getVisibleEnabledSubgraph(Vertex vertex, int depth) {
		...
		// if the given starting vertex is not visibile or enabled, returns null
		if(!vertex.visibility ||
		  (vertex instanceof DynamicVertex
				  &&
		  !((DynamicVertex) vertex).isEnabled())) return null;
		...
		this.addVisibleEnabledDepth(answer, vertex, depth, new HashSet<Vertex>());
	 ...
	 *  added to the subgraph. 
	 *  
	 *  If the given depth is set to -1, the entire enabled
	 *  graph's reaching elements are returned.
	 *  
	 *  @param vertex The starting point to obtain the subgraph.
	 *  @param depth The depth to reach when walking in depth-first mode.
	...
	public synchronized Graph getEnabledSubgraph(Vertex vertex, int depth) {
		...
		if(vertex instanceof DynamicVertex && !((DynamicVertex) vertex).isEnabled())
			return null;
	...
	/** Returns the minimum path between two given vertexes,
	 *  using the Dijkstra's algorithm.
	 *  
	 *  Only the enabled elements (vertex and edges, visible or not)
	 *  are considered to build the path.
	 *  
	...
	public synchronized Graph getEnabledDijkstraPath(Vertex begin_vertex, Vertex end_vertex) {
		...
		// for each enabled vertex of the graph, correlates it with its distance to
		...
		for(int i = 0; i < vertexes_array.length; i++) {
			...
			if(!(vertex instanceof DynamicVertex) || ((DynamicVertex) vertex).isEnabled())
				if(vertex.equals(begin_vertex))
					vertexes_with_distances_list.add(new VertexDistanceEdge(vertex, 0, null));
				else
					vertexes_with_distances_list.add(new VertexDistanceEdge(vertex, Double.MAX_VALUE, null));
		}
		
		// mounts a heap with the vertex-distance-edge trios
		Object[] vertexes_with_distances_array = vertexes_with_distances_list.toArray();
		...
		while(!heap.isEmpty()) {
			...
			VertexDistanceEdge minimum = (VertexDistanceEdge) heap.removeSmallest();
			
			// if the distance set to the minimum element is the maximum possible double value,
			// return null (i.e. the graph is disconnected, and the end_vertex is unreachable)
			...
			for(int i = 0; i < neighbourhood.length; i++) {
				...
				if(!(neighbourhood[i] instanceof DynamicVertex) || ((DynamicVertex) neighbourhood[i]).isEnabled()) {
					// obtains all the edges between the current vertex and the current
					// neighbour, of which emitter is the current vertex
					Edge[] edges = current_vertex.getConnectingOutEdges(neighbourhood[i]);
					...
					for(int j = 0; j < edges.length; j++)
						if(edges[j].is_enabled && edges[j].getLength() < smallest_length) {
					...
					if(smallest_edge != null) {
						// obtains the vertex-distance-edge trio of the current neighbour
						...
						// verifies if it's necessary to atualize the neighbour's trio
						if(neighbour_with_distance.distance > minimum.distance + smallest_edge.getLength()) {
							neighbour_with_distance.distance = minimum.distance + smallest_edge.getLength();
		...
		while(true) {
			...
			if(next_edge != null) {
				...
				Edge next_edge_copy = next_edge.getCopy(current_vertex_copy, next_vertex_copy);
		...
		// returns the answer
		if(answer.edges.isEmpty()) answer.edges = null;
		return answer;
	}
	...
	private synchronized Graph getVisibleEnabledGraph(Vertex starting_vertex) {
		...
		if(!starting_vertex.visibility ||
		  (starting_vertex instanceof DynamicVertex
				  &&
		  !((DynamicVertex) starting_vertex).isEnabled())) return null;
		...
		while(pending_vertexes.size() > 0) {
			...
			if(!expanded_vertexes.contains(current_vertex)) {
				...
				// for each neighbour
				for(int i = 0; i < neighbourhood.length; i++) {
					// if the current neighbour is visible and is enabled
					if(neighbourhood[i].visibility &&
					  ...
					  ((DynamicVertex) neighbourhood[i]).isEnabled())) {
						...
						if(!expanded_vertexes.contains(neighbourhood[i])) {
							...
							// registers if there's some visivible and enabled edge between
	 ...
	 *  
	 *  Only the visible and enabled elements (vertexes and edges)
	...
	private synchronized void addVisibleEnabledDepth(Graph subgraph, Vertex starting_vertex, int depth, Set<Vertex> already_expanded_vertexes) {
		...
		if(!starting_vertex.visibility ||
		  (starting_vertex instanceof DynamicVertex
				  &&
		  !((DynamicVertex) starting_vertex).isEnabled())) return;
				...
				for(int i = 0; i < neighbourhood.length; i++) {
					...
					if(neighbourhood[i].isVisible() &&
						...
						if(!already_expanded_vertexes.contains(neighbourhood[i])) {
							...
							// registers if some of the connecting edges is visible and enabled
							boolean visible_edge_exists = false;
							
							// for each edge
							for(int j = 0; j < edges.length; j++) {
								// if the current edge is visible and enabled
								if(edges[j].isVisible() && edges[j].isEnabled()) {
									visible_edge_exists = true;
							...
							if(visible_edge_exists) {
								...
								// neighbour
								this.addVisibleEnabledDepth(subgraph, neighbourhood[i], depth - 1, already_expanded_vertexes);
							}
						}
					}
				}
	 ...
	 *  of already expanded vertexes must be informed.
	 *  
	 *  Only the enabled elements (vertexes and edges, visible or not)
	 *  are respectively expanded and considered.
	 *  
	 *  @param subgraph The subgraph to be expanded.
	 *  @param starting_vertex The vertex where the expansion is started.
					...
					if(!(neighbourhood[i] instanceof DynamicVertex)
					  ...
					  ((DynamicVertex) neighbourhood[i]).isEnabled()) {
						...
						if(!already_expanded_vertexes.contains(neighbourhood[i])) {
							...
							// registers if some of the connecting edges is enabled
							boolean enabled_edge_exists = false;
							...
							if(enabled_edge_exists) {
								...
								// calls this method recurvely, starting from the current
								// neighbour
								this.addEnabledDepth(subgraph, neighbourhood[i], depth - 1, already_expanded_vertexes);
							}
						}
					}
				}
...
/** Internal class that holds together a vertex,
 *  the distance of the path to reach it from another
...
final class VertexDistanceEdge implements Comparable {
	...
	/** Constructor.
	 * 
	 *  @param vertex The vertex of which distance is being held.
