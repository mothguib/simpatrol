...
public final class DynamicVertex extends Vertex implements Dynamic {
	 ...
	 * @param enabling_tpd The time probability distribution for the vertex enabling.
	 * @param disabling_tpd The time probability distribution for the vertex disabling.
	 * @param is_enabled TRUE, if the vertex is enabled, FALSE if not. */
	public DynamicVertex(String label, EventTimeProbabilityDistribution enabling_tpd, EventTimeProbabilityDistribution disabling_tpd, boolean is_enabled) {
		super(label);
	 ...
	 *  @return TRUE, if the edge is in the memory of enabled edges, FALSE if not. */
	...
	public void setIsEnabled(boolean is_enabled) {
		...
		// screen message
		System.out.println("[SimPatrol.Event] " + this.getObjectId() + " enabled " + this.is_enabled + ".");
		...
		if(!is_enabled) {
			...
			if(this.in_edges != null) {
				...
				for(int i = 0; i < edges_array.length; i++)
					if(((Edge) edges_array[i]).isEnabled()) {
						this.enabled_edges.add((Edge) edges_array[i]);
						((Edge) edges_array[i]).setIsEnabled(false);
					}
			}
		...
		// if is_enabled is TRUE
			...
			Object[] edges_array = this.enabled_edges.toArray();
			for(int i = 0; i < edges_array.length; i++)
				((Edge) edges_array[i]).setIsEnabled(true);
	...
	public DynamicVertex getCopy() {
		DynamicVertex answer = new DynamicVertex(this.label, this.enabling_tpd, this.disabling_tpd, this.is_enabled);
		answer.id = this.id;
	...
	public String fullToXML(int identation) {
		...
		if(!this.is_enabled) {
			...
			buffer.replace(index_enabled_value + 12, index_enabled_value + 12 + 4, "false");
		}
