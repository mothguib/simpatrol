...
public abstract class PerceptionTranslator extends Translator {
	 ...
	 *  except for the AgentsPerception perceptions and
	 *  for the StigmasPerception perceptions.
	 *  
	 *  To obtain perceptions of agents,
	 *  use getAgentsPerceptions(Element xml_element, Graph graph).
	 *  @see AgentsPerception
	 *  
	 *  To obtain perceptions of stigmas,
	 *  use getStigmasPerceptions(Element xml_element, Graph graph).
	...
	/** Obtains the perceptions of agents (AgentsPerception objects)
	 *  from the given XML element.
	 *  
	 *  @param xml_element The XML source containing the perceptions.
	 *  @param graph The previously perceived graph of the simulation.
	 *  @return The perceptions from the XML source. */
	public static AgentsPerception[] getAgentsPerceptions(Element xml_element, Graph graph) {
		// obtains the nodes with the "perception" tag
		NodeList perception_node = xml_element.getElementsByTagName("perception");
		...
		for(int i = 0; i < perception_node.getLength(); i++) {
			...
			Agent[] agents = SocietyTranslator.getAgents(perception_element, false, graph);
	...
	/** Obtains the perceptions of stigmas (StigmasPerception objects)
	 *  from the given XML element.
	 *  
	 *  @param xml_element The XML source containing the perceptions.
	 *  @param graph The previously perceived graph of the simulation.
	 *  @return The perceptions from the XML source. */
	public static StigmasPerception[] getStigmasPerceptions(Element xml_element, Graph graph) {
		// obtains the nodes with the "perception" tag
		NodeList perception_node = xml_element.getElementsByTagName("perception");
