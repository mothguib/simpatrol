...
public class HS_OpenCycledClient extends Client {
	String env_file;
	 ...
	 */
	public HS_OpenCycledClient(String remote_socket_address,
					...
					if(!soc.id.equals("InactiveSociety"))
						for (int i = 0; i < soc.agents.length; i++) {
							...
							if (agent_ids[corresponding_id].equals("coordinator"))
								agent = new HS_OpenCycledCoordinator(hs_graph);
							else
								agent = new HS_OpenCycledAgent(soc.agents[i].id, -1, soc.agents[i].quit_time, soc.agents[i].Society_to_join);
						...
						for (int i = 0; i < soc.agents.length; i++) {
							...
							if (agent_ids[corresponding_id].equals("coordinator"))
								...
								agent = new HS_OpenCycledAgent(soc.agents[i].id, soc.agents[i].enter_time, soc.agents[i].quit_time, soc.agents[i].Society_to_join);
	...
	public static void main(String[] args) {
		...
		try {
			...
			HS_OpenCycledClient client = new HS_OpenCycledClient(
			...
			System.out
					.println("Usage \"java cycled.OpenCycledClient.java\n"
