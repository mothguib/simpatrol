...
public class HS_OpenHPCCClient extends Client {
	String env_file;
	int max_hops;
	 ...
	 */
	public HS_OpenHPCCClient(String remote_socket_address,
			int remote_socket_number, String environment_file_path, int max_graph_hops,
			String log_file_path, int time_of_simulation,
			boolean is_real_time_simulator, boolean coord_hack) throws UnknownHostException,
			IOException {
	...
	public static void main(String[] args) {
		System.out.println("Open Heuristic cognitive coordinated agents, High scale version");

		try {
			String remote_socket_address = args[0];
			int remote_socket_number = Integer.parseInt(args[1]);
			String environment_file_path = args[2];
			int max_graph_hops = Integer.parseInt(args[3]);
			String log_file_path = args[4];
			int time_of_simulation = Integer.parseInt(args[5]);
			boolean is_real_time_simulator = Boolean.parseBoolean(args[6]);
			boolean COORD_HACK = Boolean.parseBoolean(args[7]);

			HS_OpenHPCCClient client = new HS_OpenHPCCClient(
					remote_socket_address, remote_socket_number,
					environment_file_path, max_graph_hops, log_file_path, time_of_simulation,
					is_real_time_simulator, COORD_HACK);
			client.start();
		} catch (Exception e) {
			System.out
					.println("Usage \"java heuristic_cognitive_coordinated.OpenHPCCClient.java\n"
							+ "<IP address> <Remote socket number> <Environment file path>\n"
							+ "<field of view of the coordinator, in nb of nodes away. for the similar features on agents, please use the environment file>\n"
							+ "<Log file name> <Time of simulation> <Is real time simulator? (true | false)>"
							+ "<Are messages instantaneous ? (true | false)>\"");
		}
	}
}
