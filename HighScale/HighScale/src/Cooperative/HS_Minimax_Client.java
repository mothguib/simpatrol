...
public class HS_Minimax_Client  extends Client {
	...
	public HS_Minimax_Client(String remote_socket_address, int remote_socket_number,
			String environment_file_path, String log_file_path,
			double time_of_simulation, boolean is_real_time_simulator, 
			double idleness_rate_for_path, double distance, int hops, int delay)
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new HS_Minimax_Agent(soc.agents[i].id, hs_graph, -1, soc.agents[i].quit_time, 
														mynodes, 
														idleness_rate_for_path, this.max_distance, soc.agents[i].Society_to_join, 
														max_hops, transaction_delay);
				...
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new HS_Minimax_Agent(ordered_agents[i].id, hs_graph, ordered_agents[i].enter_time, ordered_agents[i].quit_time, 
																new LinkedList<String>(), 
																idleness_rate_for_path, this.max_distance, ordered_agents[i].Society_to_join,
																max_hops, transaction_delay);
	...
	public static void main(String[] args) {
		System.out.println("Open Minimax Social Agents ");
		...
		try {
			...
			double max_distance = Double.parseDouble(args[7]);
			int max_hops = Integer.parseInt(args[8]);
			int transaction_delay = Integer.parseInt(args[9]);
			
			HS_Minimax_Client client = new HS_Minimax_Client(
					remote_socket_address, remote_socket_number,
					environment_file_path, log_file_path, time_of_simulation,
					is_real_time_simulator, idleness_rate_for_path, max_distance, 
					max_hops, transaction_delay);
			client.start();
		} catch (Exception e) {
			System.out
					.println("Usage \"java Cooperative.HS_Minimax_Client \n"
							+ "<IP address> <Remote socket number> <Environment file path>\n"
							+ "<Log file name> <Time of simulation> <Is real time simulator? (true | false)>\"\n"
							+ "<rate of idleness in calculating heuristic destination><max distance for entry/exit>\n"
							+ "<distance in hops around the agent for its field of vision><auction delay between emission and attribution>");
		}
	}
