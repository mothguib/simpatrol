...
public class HS_Minimax_Agent extends HS_Cooperative_Agent {
	
	protected double MAX_DIST;
	
	

	public HS_Minimax_Agent(String id, HS_Graph completeGraph,
			LinkedList<String> nodes, double idleness_rate_for_path, double max_dist, int max_search_hops, int transaction_delay) {
		super(id, completeGraph, nodes, idleness_rate_for_path, max_search_hops, transaction_delay);
		MAX_DIST = max_dist;
	}
	
	public HS_Minimax_Agent(String id, HS_Graph completeGraph, double entering_time, double quitting_time, 
			 LinkedList<String> nodes,
			double idleness_rate_for_path, double max_dist, String society_id, int max_search_hops, int transaction_delay) {
		super(id, completeGraph, entering_time, quitting_time, nodes, idleness_rate_for_path, society_id, max_search_hops, transaction_delay);
		MAX_DIST = max_dist;
	}
	...
	protected LinkedList<String> GenerateAuctionLaunchMessage(LinkedList<String> proposed_nodes) {
		...
		for(String node : proposed_nodes)
			new_list.add(node);
		new_list.add(Double.toString(this.current_utility));
		return new_list;
	}
	...
	protected ComplexBid emitProposition(LinkedList<String> received_offers) {
		...
		proposer_utility = Double.valueOf(received_offers.get(received_offers.size() - 1));
		...
		/*
		// here we check that the agent knows the nodes that are proposed
		if(graph.getNode(offers[0]) == null && (offers.length == 1 || graph.getNode(offers[1]) == null))
			return new ComplexBid(null, null, null, null);
		else if(graph.getNode(offers[0]) == null)
			offers = new String[]{node2};
		else if(offers.length == 2 && graph.getNode(offers[1]) == null)
			offers = new String[]{node1};
		*/
		...
		if((exchanges.getBidsForFirst() != null && exchanges.getBidsForFirst().length > 0)
        	...
        	for(int k = 0; k < offers.length; k++){
    			...
    			double new_utility = this.CalculateUtility(this.path_calculator.GetShortestPath(complete_graph, new_set));
        		
    			if(new_utility < proposer_utility){
    				if(k == 0)
    		...
    		for(int i = 0; i < proposed_exchanges.size(); i++){
    			...
    			if(prop_for_1 != null && prop_for_1.length > 0){
    				for(int k = 0; k <  prop_for_1.length; k+=3){
    					//if(graph.getNode(prop_for_1[k+1]) != null){
	    					for(int j = 0; j < this.myNodes.size(); j++){
	    					...
	    					// with node offered
	    					actual_utility = this.CalculateUtility(this.path_calculator.GetShortestPath(complete_graph, new_set));
    			...
    			if(prop_for_2 != null && prop_for_2.length > 0 && best_proposition2 != null){
    				for(int k = 0; k <  prop_for_2.length; k+=3){
    					//if(graph.getNode(prop_for_2[k+1]) != null){
	    					for(int j = 0; j < this.myNodes.size(); j++){
    			...
    			if(best_proposition2 != null && prop_for_both != null && prop_for_both.length > 0){
    				for(int k = 0; k <  prop_for_both.length; k+=3){
    					//if(graph.getNode(prop_for_both[k+1]) != null){
	    					for(int j = 0; j < this.myNodes.size(); j++){
    			...
    			if(double_prop != null && double_prop.size() > 0){
    				for(int k = 0; k <  double_prop.size(); k++){
    					...
    					//if(graph.getNode(subprop[1]) != null && graph.getNode(subprop[3]) != null){
	    					for(int j = 0; j < this.myNodes.size(); j++){
    	    		...
    	    		for(int i = 0; i < FBAbuyers_exchanges.size(); i++){
    					...
    					if(i != best_buyer){
    						...
    						if(best_offer1 != null && best_offer2 != null){
    							...
    							act = new HS_SpeechAct(this.myCurrentTransaction, SpeechActPerformative.ACCEPT, this.agent_id, buyer, troc, -1);
	...
	@Override
	protected HS_SpeechAct enter_Message(int id) {
		LinkedList<String> my_position = new LinkedList<String>();
		my_position.add(this.current_position.STRING);
		return new HS_SpeechAct(id, SpeechActPerformative.ENTER, this.agent_id, "all_agents", my_position, 1, (int)this.time + this.transaction_DL);
	}
	 ...
	 */
	protected HS_SpeechAct manage_Enter_Message(HS_SpeechAct entering_act) {
		...
		for(int j = 0; j < this.myNodes.size(); j++){
			...
			if(!this.isNodeEngaged(current.STRING))
				if(this.complete_graph.getDistance(this.complete_graph.getNode(agent_position), this.complete_graph.getNode(current.STRING)) <= this.MAX_DIST){
					pathlist = this.getMyNodes();
					pathlist.remove(current.STRING);
					path_utility = this.CalculateUtility(this.path_calculator.GetShortestPath(complete_graph, pathlist));
		...
		if(nodes_to_send.length != 0){
			...
			for(int i = 0; i < this.myNodes.size(); i++){
				if(!this.isNodeEngaged(this.myNodes.get(i).STRING)){
					double dist = this.complete_graph.getDistance(this.complete_graph.getNode(agent_position), this.complete_graph.getNode(this.myNodes.get(i).STRING));
					if( dist < min_dist){
						index = i;
						min_dist = dist;
					}
		...
		HS_SpeechAct act = new HS_SpeechAct(entering_act.getTransactionId(), SpeechActPerformative.PROPOSE, 
																	this.agent_id, entering_act.getSender(), bid, -1);
		...
		if(this.ComplexPropositions != null && this.ComplexPropositions.size()>0){
			...
			for(int i = 0; i < this.ComplexPropositions.size(); i++){
				...
				if(nodesToAdd != null && nodesToAdd.length > 0){
					...
					HS_SpeechAct act = new HS_SpeechAct(this.myCurrentTransaction, SpeechActPerformative.ACCEPT, 
							this.agent_id, this.FBABuyers.get(i), -1);
					this.SendSpeechAct(act);
			...
			while(this.myNodes.size() < 4 && second_choices_agents.size() > 0){
				...
				for(int i = 0; i < second_choices.size(); i++){
					...
					double path_utility = this.CalculateUtility(this.path_calculator.GetShortestPath(complete_graph, pathlist));
				...
				if(index != -1){
					...
					HS_SpeechAct act = new HS_SpeechAct(this.myCurrentTransaction, SpeechActPerformative.ACCEPT, 
							this.agent_id, second_choices_agents.get(index), -1);
					this.SendSpeechAct(act);
			...
			for(int i = 0; i < second_choices_agents.size(); i++){
				HS_SpeechAct act = new HS_SpeechAct(this.myCurrentTransaction, SpeechActPerformative.REJECT, 
						this.agent_id, second_choices_agents.get(i), -1);
				this.SendSpeechAct(act);
			}
	...
	@Override
	protected HS_SpeechAct quit_Message(int id) {
		String[] nodes = new String[this.myNodes.size()];
		for(int i = 0; i < this.myNodes.size(); i++)
			nodes[i] = this.getMyNodes(i);
		
		ComplexBid bid = new ComplexBid(nodes, null, null, null);
		HS_SpeechAct act = new HS_SpeechAct(id, SpeechActPerformative.QUIT, this.agent_id, "all_agents", bid, (int)this.time + this.transaction_DL);
		
		return act;
	}

	@Override
	protected HS_SpeechAct manage_Quit_Message(HS_SpeechAct quitting_act) {
		...
		for(int i = 0; i < proposed_nodes.length; i++){
			if(this.get_minDistance_to_nodes(proposed_nodes[i], nodes) < this.MAX_DIST){
				...
				utility = this.CalculateUtility(this.path_calculator.GetShortestPath(complete_graph, nodes));
		...
		return new HS_SpeechAct(quitting_act.getTransactionId(), SpeechActPerformative.PROPOSE, this.agent_id, 
									quitting_act.getSender(), mybid, -1);
	}
	...
	protected void manage_Quit_Protocol() {
    	...
    	for(int j = 0; j < agents.length; j++){
    		...
    		HS_SpeechAct act = new HS_SpeechAct(myCurrentTransaction, SpeechActPerformative.ACCEPT, this.agent_id, agents[j], bid, -1);
			this.SendSpeechAct(act);
    	}
	...
	protected double get_minDistance_to_nodes(String target, LinkedList<String> nodes){
		double min_dist = Double.MAX_VALUE;
		Node target_node = this.complete_graph.getNode(target);
		
		for(int i = 0; i < nodes.size(); i++){
			double dist = this.complete_graph.getDistance(target_node, this.complete_graph.getNode(nodes.get(i)));
			if(dist < min_dist)
				min_dist = dist;
		}
	...
	protected double get_minDistance_to_nodes(String target, String[] nodes, int last_index){
		...
		for(int i = 0; i < last_index; i++){
			double dist = this.complete_graph.getDistance(target_node, this.complete_graph.getNode(nodes[i]));
			if(dist < min_dist)
				min_dist = dist;
		}
