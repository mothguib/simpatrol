...
public class NormalEventTimeProbabilityDistributionJPanel extends EventTimeProbabilityDistributionJPanel {    
	/* Methods. */
	/** Constructor.
     * 
     *  @param etpd The NormalEventTimeProbabilityDistribution object to be configured. */
    public NormalEventTimeProbabilityDistributionJPanel(NormalEventTimeProbabilityDistribution etpd) {
        this.initComponents();
    ...
    private void initComponents(NormalEventTimeProbabilityDistribution etpd) {
        ...
        this.std_field.setText(String.valueOf(this.etpd.getStandard_deviation()));
    ...
    /** Executed when the std_field changes.
     *  Generated by NetBeans IDE 3.6. */
        ...
        if(str_std.length() > 0)
            try {
                ...
                if(str_std.charAt(str_std.length() - 1) != '.') {
                    JOptionPane.showMessageDialog(this, "The standard deviation must be a real number.", "Standard deviation error.", JOptionPane.ERROR_MESSAGE);
                    this.std_field.setText("0");
                }
            }
    }//GEN-LAST:event_std_fieldKeyReleased
    
    /** Executed when the mean_field changes.
     *  Generated by NetBeans IDE 3.6. */
        ...
        if(str_mean.length() > 0)
            try {
                ...
                if(str_mean.charAt(str_mean.length() - 1) != '.') {
                    JOptionPane.showMessageDialog(this, "The mean value must be a real number.", "Mean value error.", JOptionPane.ERROR_MESSAGE);
    ...
    public model.etpd.EventTimeProbabilityDistribution getETPD() {
    	try {
            ...
            this.etpd = new NormalEventTimeProbabilityDistribution(seed, mean, std);
        }
        catch(NumberFormatException e) {
        	JOptionPane.showMessageDialog(this, "The mean and standard deviation values must be real numbers.", "Probability value error.", JOptionPane.ERROR_MESSAGE);
