...
public class SpeedLimitationJPanel extends LimitationJPanel {	
    ...
    /** Constructor.
     * 
     *  @param limitation The SpeedLimitation object to be configured. */
        ...
        if(str_speed.length() > 0)
            try {
                ...
                if(str_speed.charAt(str_speed.length() - 1) != '.') {
                    JOptionPane.showMessageDialog(this, "The speed value must be a real non-negative number.", "Value error.", JOptionPane.ERROR_MESSAGE);
	...
	public Limitation getLimitation() {
        try {
            double speed = Double.parseDouble(this.speed_field.getText().trim());
