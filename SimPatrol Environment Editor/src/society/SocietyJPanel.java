...
/**
 * Implements the GUI panel able to configure Society objects.
 * 
 * @see Society
 */
public class SocietyJPanel extends javax.swing.JPanel {
	 ...
	 * @param graph
	 *            The graph of the patroling simulation.
	 */
	...
	private void remove_buttonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_remove_buttonActionPerformed
		DefaultTableModel table_model = (DefaultTableModel) this.agents_table
				.getModel();
		...
		for (int i = 0; i < selected_rows_count; i++) {
			String agent_id = (String) this.agents_table.getValueAt(
					selected_rows[i], 0);
		...
		if (this.agents_table.getSelectedRowCount() == 1) {
			...
			for (int i = 0; i < this.agents.size(); i++)
				if (this.agents.get(i).getObjectId().equals(agent_id)) {
					this.agent_gui = new AgentGUI(this.owner, this.agents
							.get(i), this.graph);
					...
					if (obtained_agents.length > 0) {
						...
						for (int j = 0; j < obtained_agents.length; j++) {
							String[] content = { obtained_agents[j]
									.getObjectId() };
			...
			JOptionPane.showMessageDialog(this,
					"Please select only one agent.", "Selection error",
	...
	private void add_buttonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_add_buttonActionPerformed
		...
		if (this.is_closed_society)
			added_agent = new PerpetualAgent("", this.graph.getVertexes()[0],
					null, null);
		else
			added_agent = new SeasonalAgent("", this.graph.getVertexes()[0],
					null, null, null);

		added_agent.setObjectId(added_agent.getClass().getName() + "@"
				+ Integer.toHexString(added_agent.hashCode()) + "#"
	...
	private void label_fieldKeyReleased(java.awt.event.KeyEvent evt) {// GEN-FIRST:event_label_fieldKeyReleased
		...
		String id_part = null;
		StringTokenizer tokenizer = new StringTokenizer(this.id_field.getText()
				.trim(), "@", false);
		try {
			tokenizer.nextToken();
	...
	public Society getSociety() {
		...
		if (this.is_closed_society) {
			PerpetualAgent[] perpetual_agents = new PerpetualAgent[this.agents
					.size()];
			for (int i = 0; i < perpetual_agents.length; i++)
				perpetual_agents[i] = (PerpetualAgent) this.agents.get(i);

			answer = new ClosedSociety(this.label_field.getText().trim(),
					perpetual_agents);
		} else {
			SeasonalAgent[] seasonal_agents = new SeasonalAgent[this.agents
					.size()];
			for (int i = 0; i < seasonal_agents.length; i++)
				seasonal_agents[i] = (SeasonalAgent) this.agents.get(i);

			answer = new OpenSociety(this.label_field.getText().trim(),
					seasonal_agents);
		}
