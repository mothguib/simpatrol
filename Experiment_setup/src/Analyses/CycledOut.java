...
public class CycledOut {
	...
	public static void main(String[] args) {
		...
		String Log_gen_name = "/logs_Cycled/log_";
	
		AverageMetricsReport[] MyAvReports = new AverageMetricsReport[15];
		//AverageMetricsReport[] MyAvReports = new AverageMetricsReport[4];
		...
		for(int nAg = 0; nAg < MyAvReports.length; nAg++){
			...
			for(int i = 0; i < nb_log; i++){
				...
				metrics = new MetricsReport(parser.getNumNodes(), first_cycle, last_cycle, parser.getVisitsList());
		...
		int freq = 25;
		Double[] myfreq = new Double[(last_cycle-first_cycle)/freq +1];
		for(int i = 0; i < myfreq.length; i++)
			myfreq[i] = first_cycle + (double) i * freq;
		...
		myviewer.setXcenter(first_cycle);
