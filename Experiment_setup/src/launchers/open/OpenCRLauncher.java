...
import open.ConscientiousReactive.OpenCRAgent;
...
public class OpenCRLauncher extends Launcher {
	String env_file;

	public OpenCRLauncher(String environment_dir_path, String env_gen_name,
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new OpenCRAgent(soc.agents[i].id, -1, soc.agents[i].quit_time, soc.agents[i].Society_to_join);
				...
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new OpenCRAgent(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, ordered_agents[i].Society_to_join);
	...
	public static void main(String[] args) {
		System.out.println("Open CR Agents");
		...
		try {
			...
			OpenCRLauncher client = new OpenCRLauncher(
			...
			System.out
			.println("Usage \"java Open CR Agents \n"
