...
import open.HPCC.OpenHPCCAgent;
...
public class OpenHPCCLauncher extends Launcher {
	String env_file;

	public OpenHPCCLauncher(String environment_dir_path, String env_gen_name,
			int numEnv, String log_dir_path, String log_gen_name,
			int time_of_simulation, boolean coord_hack)
			...
			if(!soc.id.equals("InactiveSociety"))
				...
				for (int i = 0; i < soc.agents.length; i++) {
					...
					if (agent_ids[corresponding_id].equals("coordinator"))
						...
						agent = new OpenHPCCAgent(ordered_agents[i].id, this.IS_REAL_TIME_SIMULATOR, ordered_agents[i].enter_time, ordered_agents[i].quit_time, ordered_agents[i].Society_to_join);
	...
	public static void main(String[] args) {
		System.out.println("Open HPCC Agents");
		...
		try {
			...
			OpenHPCCLauncher client = new OpenHPCCLauncher(
					environment_dir_path, 
					env_gen_name, numEnv, 
					log_dir_path, log_gen_name, 
					time_of_simulation, COORD_HACK);
			client.start();
		} catch (Exception e) {
			System.out
			.println("Usage \"java Open HPCC Agents \n"
