...
import open.CooperativeAuctions.PathLength.Minimax_Agent_altruistic;
import open.CooperativeAuctions.PathLength.Minimax_Agent_selfish;
...
public class OpenMinimaxSelfishLauncher extends Launcher {
	...
	public OpenMinimaxSelfishLauncher(String environment_dir_path, String env_gen_name,
			int numEnv, String log_dir_path, String log_gen_name,
			int time_of_simulation,
			double idleness_rate_for_path, double max_distance)
	...
	throws IOException {
		...
		// test
		parsed_graph.getDistance(nodes[0], nodes[1]);
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Minimax_Agent_selfish(soc.agents[i].id, -1, soc.agents[i].quit_time, 
														nb_active_agents, mynodes, 
														idleness_rate_for_path, this.MAX_DISTANCE, 
				...
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Minimax_Agent_selfish(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, 
	...
	public static void main(String[] args) {
		System.out.println("Minimax Agents - path length heuristic");
		...
		try {
			...
			OpenMinimaxSelfishLauncher client = new OpenMinimaxSelfishLauncher(
					environment_dir_path, 
					env_gen_name, numEnv, 
					log_dir_path, log_gen_name, 
					time_of_simulation, idleness_rate_for_path, max_distance);
			client.start();
		} catch (Exception e) {
			System.out
					.println("Usage \"java Minimax Agents! \n"
							...
							+ "<rate of idleness in calculating heuristic destination><max distance for entry and exit mechanisms>");
		}
	}
