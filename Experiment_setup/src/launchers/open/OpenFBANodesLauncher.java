...
import open.FBA2.Open_FBA_Nodes_Agent;
...
public class OpenFBANodesLauncher extends Launcher {

	
	double idleness_rate_for_path, idleness_rate_for_auction;
	String env_file;

	public OpenFBANodesLauncher(String environment_dir_path, String env_gen_name,
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Open_FBA_Nodes_Agent(soc.agents[i].id, -1, soc.agents[i].quit_time, 
				...
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Open_FBA_Nodes_Agent(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, 
	...
	public static void main(String[] args) {
		...
		try {
			...
			OpenFBANodesLauncher client = new OpenFBANodesLauncher(
			...
			System.out
					.println("Usage \"java Open FBA Agents - coop by node strategy! \n"
