...
import open.FBA2.custom.Open_FBA_mapB_Agent;
...
public class OpenFBAmapBLauncher extends Launcher {
	...
	public OpenFBAmapBLauncher(String environment_dir_path, String env_gen_name,
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Open_FBA_mapB_Agent(soc.agents[i].id, -1, soc.agents[i].quit_time, 
														nb_active_agents, mynodes, 
														idleness_rate_for_path, idleness_rate_for_auction, this.max_dist_entry,
				...
				for (int i = 0; i < ordered_agents.length; i++) {
					...
					Agent agent = new Open_FBA_mapB_Agent(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, 
	...
	public static void main(String[] args) {
		...
		try {
			...
			OpenFBAmapBLauncher client = new OpenFBAmapBLauncher(
					environment_dir_path, 
					env_gen_name, numEnv, 
					log_dir_path, log_gen_name, 
					time_of_simulation, idleness_rate_for_path, idleness_rate_for_auction, max_dist_entry);
			client.start();
		} catch (Exception e) {
			System.out
					.println("Usage \"java Open FBA Agents - custom mapB! \n"
					...
					"<max distance to consider nodes, entry mechanism>\n");
		}
	}
