...
import open.FBA2.Open_FBA_Proximity_Agent;
import open.FBA2.custom.Open_FBA_mapA_Agent;
...
public class OpenFBAmapALauncher extends Launcher {

	
	double idleness_rate_for_path, idleness_rate_for_auction;
	double max_dist_entry;
	double max_dist_exit;
	String env_file;

	public OpenFBAmapALauncher(String environment_dir_path, String env_gen_name,
			int numEnv, String log_dir_path, String log_gen_name,
			int time_of_simulation,
			double idleness_rate_for_path, double idleness_rate_for_auction, double max_dist_entry, double max_dist_exit)
		...
		this.max_dist_entry = max_dist_entry;
		this.max_dist_exit = max_dist_exit;
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Open_FBA_mapA_Agent(soc.agents[i].id, -1, soc.agents[i].quit_time, 
														nb_active_agents, mynodes, 
														idleness_rate_for_path, idleness_rate_for_auction, this.max_dist_entry, this.max_dist_exit,
				...
				for (int i = 0; i < ordered_agents.length; i++) {
					...
					Agent agent = new Open_FBA_mapA_Agent(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, 
	...
	public static void main(String[] args) {
		System.out.println("Open FBA Agents - custom mapA !");
		...
		try {
			...
			double max_dist_entry = Double.parseDouble(args[8]);
			double max_dist_exit = Double.parseDouble(args[9]);
			
			OpenFBAmapALauncher client = new OpenFBAmapALauncher(
					environment_dir_path, 
					env_gen_name, numEnv, 
					log_dir_path, log_gen_name, 
					time_of_simulation, idleness_rate_for_path, idleness_rate_for_auction, max_dist_entry, max_dist_exit);
			client.start();
		} catch (Exception e) {
			System.out
					.println("Usage \"java Open FBA Agents - custom mapA! \n"
					...
					"<max distance to consider nodes, entry mechanism><max distance to consider nodes, exit mechanism>\n");
		}
	}
