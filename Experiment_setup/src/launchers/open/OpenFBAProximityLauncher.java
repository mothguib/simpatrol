...
public class OpenFBAProximityLauncher extends Launcher {
	...
	public OpenFBAProximityLauncher(String environment_dir_path, String env_gen_name,
			int numEnv, String log_dir_path, String log_gen_name,
			int time_of_simulation,
			double idleness_rate_for_path, double idleness_rate_for_auction, double max_dist)
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Open_FBA_Proximity_Agent(soc.agents[i].id, -1, soc.agents[i].quit_time, 
														nb_active_agents, mynodes, 
														idleness_rate_for_path, idleness_rate_for_auction, this.max_dist,
				...
				for (int i = 0; i < ordered_agents.length; i++) {
					...
					Agent agent = new Open_FBA_Proximity_Agent(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, 
	...
	public static void main(String[] args) {
		System.out.println("Open FBA Agents - coop by Proximity strategy!");
		...
		try {
			...
			double max_dist = Double.parseDouble(args[8]);
			
			OpenFBAProximityLauncher client = new OpenFBAProximityLauncher(
					environment_dir_path, 
					env_gen_name, numEnv, 
					log_dir_path, log_gen_name, 
					time_of_simulation, idleness_rate_for_path, idleness_rate_for_auction, max_dist);
			client.start();
		} catch (Exception e) {
			System.out
					.println("Usage \"java Open FBA Agents - coop by proximity strategy! \n"
					...
					"<max distance to consider nodes>\n");
		}
	}
