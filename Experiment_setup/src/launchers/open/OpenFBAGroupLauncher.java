...
public class OpenFBAGroupLauncher extends Launcher {

	
	double idleness_rate_for_path, idleness_rate_for_auction;
	String env_file;

	public OpenFBAGroupLauncher(String environment_dir_path, String env_gen_name,
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Open_FBA_Group_Agent(soc.agents[i].id, -1, soc.agents[i].quit_time, 
				...
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Open_FBA_Group_Agent(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, 
	...
	public static void main(String[] args) {
		System.out.println("Open FBA Agents - coop by group of nodes strategy!");
		...
		try {
			...
			OpenFBAGroupLauncher client = new OpenFBAGroupLauncher(
			...
			System.out
					.println("Usage \"java Open FBA Agents - coop by group of nodes strategy! \n"
