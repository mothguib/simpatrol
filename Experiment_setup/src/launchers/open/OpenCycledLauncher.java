...
import open.SingleCycle.OpenCycledAgent;
import open.SingleCycle.OpenCycledCoordinator;
...
public class OpenCycledLauncher extends Launcher {

		String env_file;

		public OpenCycledLauncher(String environment_dir_path, String env_gen_name,
				...
				if(!soc.id.equals("InactiveSociety"))
					for (int i = 0; i < soc.agents.length; i++) {
						...
						if (agent_ids[corresponding_id].equals("coordinator"))
							agent = new OpenCycledCoordinator();
						else
							agent = new OpenCycledAgent(soc.agents[i].id, -1, soc.agents[i].quit_time, soc.agents[i].Society_to_join);
					...
					for (int i = 0; i < soc.agents.length; i++) {
						...
						if (agent_ids[corresponding_id].equals("coordinator"))
							...
							agent = new OpenCycledAgent(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, ordered_agents[i].Society_to_join);
		...
		public static void main(String[] args) {
			System.out.println("Open Cycled Agents");
			...
			try {
				...
				OpenCycledLauncher client = new OpenCycledLauncher(
				...
				System.out
						.println("Usage \"java Open Cycled Agents \n"
