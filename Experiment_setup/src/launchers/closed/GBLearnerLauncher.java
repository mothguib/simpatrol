...
import closed.gray_box_learner.MyGBLearnerAgent;
...
public class GBLearnerLauncher extends Launcher {
	 ...
	 */
	public GBLearnerLauncher(String environment_dir_path, String env_gen_name,
		...
		super(environment_dir_path, 
				...
				time_of_simulation);
		MyGBLearnerAgent.configureLearningEngine(e, alfa_decay_rate, gama,
				state_items_cardinality, actions_per_state_count);
		MyGBLearnerAgent.setQtableDirectory(q_table_dir);
		MyGBLearnerAgent.setIsLearningPhase(is_learning_phase);
		MyGBLearnerAgent.GENERALIZED = generalized;
	}
	...
	protected void createAndStartAgents(String[] agent_ids, int[] socket_numbers)
		...
		for (int i = 0; i < agent_ids.length; i++) {
			MyGBLearnerAgent agent = new MyGBLearnerAgent();
	...
	public static void main(String[] args) {
		...
		try {
			...
			GBLearnerLauncher client = new GBLearnerLauncher(
			...
			System.out
					.println("Usage \"java launchers.GBLearnerLauncher\n"
