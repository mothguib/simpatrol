...
import strategies.cc.CcCoordinatorAgent;
import strategies.cc.CcCoordinatorAgentOld;
 ...
 */
public class CcLauncher extends Launcher {
	private static final String USAGE = Launcher.USAGE.replaceFirst("\\[CLIENT_CLASS\\]", CcLauncher.class.getSimpleName())
	 ...
	 */
	public CcLauncher(String[] args) throws Exception {
		super(args);
	}
	...
	protected void createAndStartAgents(String[] agentIds, int[] portNumbers)
		...
		for (int i = 0; i < agentIds.length; i++) {
			...
			if (agentIds[i].equals("coordinator")) {
				agent = new CcCoordinatorAgent(agentIds[i], connection);
	...
	public static void main(String[] args) {
		...
		try {
			CcLauncher client = new CcLauncher(args);
			client.start();		
		} catch (Exception e) {
