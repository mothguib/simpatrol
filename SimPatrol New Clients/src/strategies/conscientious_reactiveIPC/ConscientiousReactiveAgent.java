...
public class ConscientiousReactiveAgent extends Agent {
	...
	// replaces the connection from superclass
	ClientConnection connection2;

	/** Configures the connection of the agent. */
	public void setConnection2(ClientConnection connection) {
	...
	public void run() {
		...
		while (!this.stop_working) {
			...
			while (current_position == null || neighbourhood.length == 0) {
				...
				String[] perceptions = this.connection2.getBufferAndFlush();
			...
			//Workaround bug lastGoNode
			
			if ((next_vertex_id != null && !next_vertex_id.equals(lastGoNode)) || lastGoNode.equals("")) {
