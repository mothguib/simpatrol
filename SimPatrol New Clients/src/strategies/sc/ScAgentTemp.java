 ...
 * [2005].
 */
public final class ScAgentTemp extends ThreadAgent {
	...
	/** Constructor. */
	public ScAgentTemp(String identifier, IpcConnection actualConnection) {
		super(identifier, new IpcConnection("nvvaejane3svowivaenrgo3og"), false);
		this.connection = actualConnection;
		this.id = identifier;
				...
				//if (this.connection instanceof TCPClientConnection || this.connection instanceof IPCConnection) {
					if (this.cycles_count - this.start_time > this.wait_time) {
						...
						try {
							...
							System.err.println("Here I go: "+this.id+" On Cycle "+this.cycles_count);
						} catch (IOException e) {
							e.printStackTrace();
						}
