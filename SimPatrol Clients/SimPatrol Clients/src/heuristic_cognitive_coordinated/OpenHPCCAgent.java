...
public class OpenHPCCAgent extends OpenAgent {
	...
	protected void active_run() {
		if(this.PLAN.size() == 0){
			...
			// coordinator
			int begin_waiting_time = Calendar.getInstance().get(Calendar.SECOND);
			...
			while ((goal_node == null || current_graph == null || !time_actualized) && !this.stop_working) {
				// obtains the perceptions sent by SimPatrol server
				String[] perceptions = this.connection.getBufferAndFlush();
		...
		while ((current_position == this.current_position || current_position.double_value != 0 ) && !this.stop_working) {
			...
			// previous one
			if (current_position != null && !current_position.equals(this.current_position))
