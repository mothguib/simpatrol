...
public class Open_FBA_coopByNode_Client  extends Client {
	double idleness_rate_for_path, idleness_rate_for_auction;
	String env_file;
	

	public Open_FBA_coopByNode_Client(String remote_socket_address, int remote_socket_number,
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Open_FBA_coopByNode_Agent(soc.agents[i].id, -1, soc.agents[i].quit_time, 
				...
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Open_FBA_coopByNode_Agent(soc.agents[i].id, soc.agents[i].enter_time, soc.agents[i].quit_time, 
	...
	public static void main(String[] args) {
		...
		try {
			...
			Open_FBA_coopByNode_Client client = new Open_FBA_coopByNode_Client(
			...
			System.out
					.println("Usage \"java FBA_Open.Open_FBA_coopByNode_Client \n"
