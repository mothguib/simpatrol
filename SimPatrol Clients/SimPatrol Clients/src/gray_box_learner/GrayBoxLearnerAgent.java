...
public class GrayBoxLearnerAgent extends Agent {
	...
	private double goToAndSendMessage(int action_id) throws IOException {
		...
		if (this.na - 1 == action_id)
			...
			return Math.pow(goal_node_idleness.double_value, 2);
		} else
			return goal_node_idleness.double_value;
	}
	...
	public void run() {
		...
		this.learning_engine = new QLearningEngine(state_item_values, GrayBoxLearnerAgent.q_table_dir + this.id + ".txt");
