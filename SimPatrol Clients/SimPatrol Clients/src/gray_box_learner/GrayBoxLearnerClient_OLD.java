 ...
 */
public final class GrayBoxLearnerClient_OLD extends Client_OLD {
	/* Methods. */
	/**
	 * Constructor.
	 * 
	 ...
	 */
	public GrayBoxLearnerClient_OLD(String remote_socket_address,
		...
		GrayBoxLearnerAgent_OLD.configureLearningEngine(e, alfa_decay_rate, gama,
				state_items_cardinality, actions_per_state_count);
		GrayBoxLearnerAgent_OLD.setQtableDirectory(q_table_dir);
		GrayBoxLearnerAgent_OLD.setIsLearningPhase(is_learning_phase);
		GrayBoxLearnerAgent_OLD.GENERALIZED = generalized;
	}
	...
	protected void createAndStartAgents(String[] agent_ids, int[] socket_numbers)
		...
		for (int i = 0; i < agent_ids.length; i++) {
			GrayBoxLearnerAgent_OLD agent = new GrayBoxLearnerAgent_OLD();
	...
	public static void main(String[] args) {
		...
		try {
			...
			GrayBoxLearnerClient_OLD client = new GrayBoxLearnerClient_OLD(
