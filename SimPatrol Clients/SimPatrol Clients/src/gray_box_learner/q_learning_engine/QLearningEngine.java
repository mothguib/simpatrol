...
public class QLearningEngine extends Thread {
	 ...
	 */
	public QLearningEngine(QState qstate, String q_table_file_path) {
		this.is_active = false;

		current_qstate = qstate;
		...
		try {
			...
			System.out.println("Q_table successfully loaded");
		} catch (IOException e) {
			this.q_table = new QTable(states_count,
					QLearningEngine.actions_per_state_count,
					QLearningEngine.configuration.getE());
			System.out.println("Q_table successfully created");
		}

		this.current_state_id = current_qstate.getStateId();
	 ...
	 *            the environment.
	 */
	public void setNextState(QState qstate) {
		current_qstate = qstate;
		this.next_state_id = current_qstate.getStateId();
		updated = false;
	}
	...
	private void updateQTable() {
		if (QLearningEngine.is_learning_phase) {
			double current_value = this.q_table.getValue(this.current_state_id, this.last_action_id);

			double alfa = Math.pow((2 + this.q_table.getUse(this.current_state_id, this.last_action_id)
											* Math.pow(QLearningEngine.configuration.getAlfa_decay_rate(), -1)),
										-1);
