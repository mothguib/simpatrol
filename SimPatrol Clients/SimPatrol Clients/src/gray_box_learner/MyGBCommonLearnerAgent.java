...
import gray_box_learner.q_learning_engine.QState;
import gray_box_learner.q_learning_engine.QStateSantana2004;
...
public class MyGBCommonLearnerAgent extends Agent {
	/* Attributes. */
	...
	private CommonQLearningEngine learning_engine;
	...
	/* Methods. */
	/** Constructor. */
	public MyGBCommonLearnerAgent(CommonQLearningEngine CQL) {
		this.learning_engine = CQL;
	...
	public void run() {
		...
		this.learning_engine.initAgent(this.id, state_values);
		...
		while (!this.stop_working) {
			...
			this.learning_engine.setPossibleActionsCount(this.id, this.neighborhood.size());

			// holds the reward for the current action
			double reward = 0;

			// executes the next action (the -1 compensates a +1 in the QLearning_engine...
			int action_id = this.learning_engine.getActionId(this.id) - 1;
			try {
			...
			this.learning_engine.setLastActionDuration(this.id, this.last_action_duration);
			System.err.println("Action duration " + this.last_action_duration);

			// configures the reward of the action
			this.learning_engine.setReward(this.id, reward + this.last_action_duration);

			// mounts the array with the next state item values and configures
			// it into the engine
			QState next_item_values = this.getQStateValue();
			this.learning_engine.setNextState(this.id, next_item_values);
			
			
			while(!this.learning_engine.is_updated(this.id))
				try {
					sleep(2);
