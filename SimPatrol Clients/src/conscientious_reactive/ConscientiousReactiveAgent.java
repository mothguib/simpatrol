...
/** Implements the conscientoius reactive agents, as it is described
 *  in the work of [MACHADO, 2002]. */
public abstract class ConscientiousReactiveAgent extends Thread {
	/* Attributes. */
		...
		for(int i = perceptions_count - 1; i > -1; i--) {
			...
			if(current_perception.indexOf("<perception type=\"0\"") > -1) {
				...
				while(next_vertex_index > -1) {				
					...
					// adds the id of the vertex to the list of obtained vertexes
	 ...
	 *  the given current position, and the idleness memorized by the agent.
	 *  
	 *  The id of the next chosen vertex is returned.
	 *  
	 *  @param current_position The current position of the agent.
	 *  @param neighbourhood The neighbourhood of where the agent is.
	 *  @return The next vertex the agent must take, if needed. */
	private String decideNextVertex(StringAndDouble current_position, String[] neighbourhood) {
		...
		for(int i = 0; i < neighbourhood.length; i++) {
			...
			if(!vertex_id.equals(current_vertex_id)) {
				// registers if the obtained vertex is in the memory of the agent
				...
				for(int j = 0; j < this.vertexes_idlenesses.size(); j++) {
					StringAndDouble memorized_item = this.vertexes_idlenesses.get(j);
					...
					if(memorized_item.STRING.equals(vertex_id)) {
						...
						// registers that the obtained vertex is already memorized
						is_memorized = true;
				...
				if(!is_memorized)
					...
					this.vertexes_idlenesses.add(new StringAndDouble(vertex_id, current_visiting_time));
				
				// if the current visiting time is smaller than the oficial visiting time
	 ...
	 * 
	 *  @param current_position The current position of the agent, to be visited.  
	 *  @throws IOException */
	private void visitCurrentPosition(StringAndDouble current_position) throws IOException {
	...
	public void run() {
		...
		while(!this.stop_working) {
			...
			while(current_position == null || neighbourhood.length == 0) {
				...
				String[] current_neighbourhood = this.perceiveNeighbourhood(perceptions);
