/* HeuristicConscientiousReactiveClient.java */
 ...
 * letting heuristic conscientious reactive agent clients connect to it, in the
 * sequence.
 */
public class HeuristicConscientiousReactiveClient extends Client {
	/**
	 * Constructor.
	 * 
	 ...
	 */
	public HeuristicConscientiousReactiveClient(String remote_socket_address,
	...
	protected void createAndStartAgents(int[] socket_numbers)
		...
		for (int i = 0; i < socket_numbers.length; i++) {
			HeuristicConscientiousReactiveAgent agent = null;

			if (this.IS_REAL_TIME_SIMULATOR)
				agent = new RealTimeHeuristicConscientiousReactiveAgent(
						this.CONNECTION.getRemoteSocketAdress(),
						socket_numbers[i]);
			else
				agent = new CycledHeuristicConscientiousReactiveAgent(
						this.CONNECTION.getRemoteSocketAdress(),
						socket_numbers[i]);
