 ...
 */
public abstract class Client implements IMessageObserver{
	/* Attributes. */
	...
	private int[] configureMetrics() throws IOException {
		...
		if(! silentMode ){
			...
			if (key.equalsIgnoreCase("y")) create = true;
		} else {
			if( METRICS_FILE_PATHS[0].length() > 0 ) create = true;
		}
		...
		System.out
			...
			if( METRICS_FILE_PATHS[1].length() > 0 ) create = true;
		}
		...
		if(! silentMode ){
			...
			if( METRICS_FILE_PATHS[2].length() > 0 ) create = true;
		}
	...
	private int configureLogging() throws IOException {
		...
		if( !silentMode ){
			...
			String key = Keyboard.readLine();
			if (key.equalsIgnoreCase("y")) log = true;
		} 
		if (log) {
	...
	private void createAndStartLogClient(int socket_number) throws IOException {
		...
		if (socket_number > -1) {
			...
			if(!silentMode){
				...
				if( this.LOG_FILE_PATH.length() > 0 ) log = true;;
			}
			if (log) {
				// screen message
	...
	public void start(){
		...
		System.out
				...
				if (key.equalsIgnoreCase("y")) start = true;
		} else start = true;
		if (start ) {
			try {
