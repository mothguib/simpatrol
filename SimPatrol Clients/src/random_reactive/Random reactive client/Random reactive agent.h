...
public:
	...
	virtual unsigned long run() {
		...
		while(_continue) {
			...
			// the current vertex of the agent
			...
			while(elapsed_length == -1 || current_vertex.length() == 0 || neighbourhood.size() == 0) {
				...
				if(perception.find("<perception type=\"0\"", 0) != string::npos)
					...
					current_vertex = getCurrentVertexId(perception);
				}
			}
			...
			if(elapsed_length == 0) {
				...
				string next_vertex = current_vertex;
				while(next_vertex.compare(current_vertex) == 0) {
					int index = rand() % neighbourhood.size();
					next_vertex = neighbourhood[index];
				}
				...
				string message_2 = "<action type=\"1\" vertex_id=\"";
				       message_2.append(next_vertex);
					   message_2.append("\"/>\n");
				if(is_real_time)
...
private:
	vector<string> getNeighbourhood(string perception) {
		...
		int next_vertex_index = perception.find("<vertex ", 0);
		int next_vertex_id_index = perception.find("id=\"", next_vertex_index);
		int end_vertex_id_index;
		while(next_vertex_index != string::npos) {
			next_vertex_id_index = next_vertex_id_index + 4;
			end_vertex_id_index = perception.find("\"", next_vertex_id_index);
			answer.push_back(perception.substr(next_vertex_id_index, end_vertex_id_index - next_vertex_id_index));
			next_vertex_index = perception.find("<vertex ", end_vertex_id_index);
			next_vertex_id_index = perception.find("id=\"", next_vertex_index);
		}
		
		return answer;
	}

	string getCurrentVertexId(string perception) {
		int vertex_id_index = perception.find("vertex_id=\"", 0) + 11;
		int end_vertex_id_index = perception.find("\"", vertex_id_index);
		return perception.substr(vertex_id_index, end_vertex_id_index - vertex_id_index);
	}
