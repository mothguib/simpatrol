...
/***************************************
 * 
 * Flexible Bidder Agent 2
 * 
 * This agents is very close to the FBA.FlexibleBidderAgent, with a few differences :
 * 
 *   - it extends CommunicatorAgent2 and thus has the same internal representation
 *   - the interest of nodes to trade is a little different, see ChooseNodesForAuction()
 *   
 *   
 * IMPORTANT : it is not able to cope with an open system. see the FBA_Open package 
 * 
 * 
 * @author Cyril Poulet
 ***************************************/
				...
				for(int j = 0; j < this.myNodes.size(); j++){
					...
					if((inserted == null)||(!inserted.equals(current.STRING))){
						if((idle_max - idle_min) > 0)
							// original version: the highest the idleness, the smallest the score of the node (so we keep it)
							// idleness = (idle_max - current.double_value) / (idle_max - idle_min);
							
							// new version : The highest the idleness, the higher the score (if we can't visit it often enough, try to trade it...)
...
//			System.out.println(message);
    ...
    protected void SendSpeechAct(SpeechAct act) throws IOException{
    	super.BroadcastMessage(act.toString());
