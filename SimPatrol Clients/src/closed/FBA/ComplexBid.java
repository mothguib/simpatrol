...
/**
 * This class represent the set of bids that can be sent between the agents.
 * 
 * @author pouletc
 *
 */
public class ComplexBid {
	...
	// nodes proposed for the first auctioned node
	private String[]  myBidsForFirst;
	// nodes proposed for the second auctioned node
	private String[]  myBidsForSecond;
	// nodes proposed for the pair of auctioned nodes
	private String[] myBidsForBoth;
	// pairs of nodes proposed for the pair of auctioned nodes
	...
	public boolean equals(ComplexBid bid){
		if(!Arrays.equals(this.myBidsForFirst, bid.myBidsForFirst))
			return false;
		if(!Arrays.equals(this.myBidsForSecond, bid.myBidsForSecond))
			return false;
		if(!Arrays.equals(this.myBidsForBoth, bid.myBidsForBoth))
			return false;
		
		if(this.myDoubleBidsForBoth != null){
			if(bid.myDoubleBidsForBoth == null)
				return false;
			if(this.myDoubleBidsForBoth.size() != bid.myDoubleBidsForBoth.size())
				return false;
			for(int i = 0; i < this.myDoubleBidsForBoth.size(); i++)
				if(!Arrays.equals(this.myDoubleBidsForBoth.get(i), bid.myDoubleBidsForBoth.get(i)))
					return false;
		}
		else if(bid.myDoubleBidsForBoth != null)
			return false;
		
		if(this.actual_idleness != bid.actual_idleness)
			return false;
		
		if(!Arrays.equals(this.utilities_myBidsForFirst, bid.utilities_myBidsForFirst))
			return false;
		if(!Arrays.equals(this.utilities_myBidsForSecond, bid.utilities_myBidsForSecond))
			return false;
		if(!Arrays.equals(this.utilities_myBidsForBoth, bid.utilities_myBidsForBoth))
			return false;
		if(!Arrays.equals(this.utilities_myDoubleBidsForBoth, bid.utilities_myDoubleBidsForBoth))
			return false;
