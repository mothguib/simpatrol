...
public abstract class ReactiveWithFlagsAgent extends Agent {
	...
	/**
	 * Lets the agent perceive the idlenesses of each vertex in the
	 * neighbourhood.
	 * 
	 * @param perceptions
	 *            The current perceptions of the agent.
	 ...
	 */
	private StringAndDouble[] perceiveNeighbourhoodIdlenesses(
			String[] perceptions) {
		...
		for (int i = perceptions_count - 1; i > -1; i--) {
			...
			if (current_perception.indexOf("<perception type=\"0\"") > -1) {
				...
				while (next_vertex_index > -1) {
					...
					int current_vertex_idleness_index = current_perception
							.indexOf("idleness=\"");
					current_perception = current_perception
							.substring(current_vertex_idleness_index + 10);
					double idleness = Double.parseDouble(current_perception
							.substring(0, current_perception.indexOf("\"")));

					// adds the pair vertex-idleness to the list of obtained
					// pairs
					vertexes_and_idlenesses.add(new StringAndDouble(vertex_id,
							idleness));
				...
				StringAndDouble[] answer = new StringAndDouble[vertexes_and_idlenesses
						.size()];
	 ...
	 * current position, and the given idlenesses of the neighbourhood.
	 * 
	...
	public void run() {
		...
		while (!this.stop_working) {
			...
			while (current_position == null || neighbourhood.length == 0) {
				...
				StringAndDouble[] current_neighbourhood = this
						.perceiveNeighbourhoodIdlenesses(perceptions);
