...
public class CycledCoordinatorAgent extends Agent {
	...
	/**
	 * The sequence of vertexes to be visited by the agents. Actually a TSP
	 ...
	 * (i+1)th position the id of the vertex where the agent is.
	 */
	...
	protected boolean perceiveTSPSolution(String perception)
		...
		if (graph_perception.length > 0) {
			...
			Vertex[] tsp_solution = this.graph.getTSPSolution();
	...
	private boolean perceiveAgentsPositions(String perception) {
		if (perception.indexOf("<perception type=\"1\"") > -1) {
			...
			while (agent_id_index > -1) {
				...
				int agent_vertex_id_index = perception.indexOf("vertex_id=\"");
				perception = perception.substring(agent_vertex_id_index + 11);
				String agent_vertex_id = perception.substring(0, perception
						.indexOf("\""));

				this.AGENTS_POSITIONS.add(agent_id);
				this.AGENTS_POSITIONS.add(agent_vertex_id);
