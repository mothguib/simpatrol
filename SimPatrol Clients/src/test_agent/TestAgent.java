		...
		while (!this.stop_working) {
			...
			if(!inactive) {
				...
				if(current_position != null && current_position.double_value == 0) {
					...
					if(this.id.equals("a1") && this.time > 10 && this.time < 30) {
						String message_1 = "<action type=\"11\"/>\n";
						try {
				...
				if( this.id.equals("a1") && this.time > 30) {
					String message_1 = "<action type=\"10\" society_id=\"s0\"/>\n";
					try {
						this.connection.send(message_1);
					} catch (IOException e) {
						// TODO Auto-generated catch block
