 ...
 *
 */
public class HybridPath implements ShortestPath {

	@Override
	public LinkedList<String> GetShortestPath(Graph graph, LinkedList<String> nodes) {
		LinkedList<String> ordered1 = this.OrderNodes(graph, nodes);
		if(ordered1.size() < 2)
			return ordered1;
		...
		Node[] ordered2 = this.Two_opt_improvement(graph, ordered1_list);
		ordered2 = this.One_target_Three_Opt(graph, ordered2);
	...
	public LinkedList<String> OrderNodes(Graph graph, LinkedList<String> nodes){
		if(nodes.size() < 2)
			return nodes;
