...
public class Graph {
	...
	protected synchronized void calculateDistances() {
		...
		for (int i = 0; i < nodes_array.length; i++) {
			...
			boolean found = false;
			for(int j = 0; j < distances_table.size(); j++)
				if (distances_table.get(j).NODE.equals(node))
					found = true;
			if(found)
				break;
	...
	public double getDistance(Node node_1, Node node_2) {
		...
		// if not found
		Graph path = this.getDijkstraPath(node_1, node_2);
		...
		for (int i = 0; i < distances_table.size(); i++){
			if (distances_table.get(i).NODE.equals(node_1)) {
				distances_table.get(i).DISTANCES_LIST.add(new NodeWithDistance(node_2, path_length));
			}
			if (distances_table.get(i).NODE.equals(node_2)) {
				distances_table.get(i).DISTANCES_LIST.add(new NodeWithDistance(node_1, path_length));
			}
		}
	...
	/**
	 * Returns the edge of the graph that links nodes n1 and n2.
	 * 
	 * @param Node n1, n2
	 *            the nodes to link (non directed).
	 * @return The edge linking the nodes, or NULL if there's no such edge.
	 */
	public Edge getEdge(Node n1, Node n2){
		if(this.edges != null)
			for(Edge edge : this.edges){
				Node[] list = edge.getNodees();
				if((list[0].getObjectId()==n1.getObjectId() && list[1].getObjectId()==n2.getObjectId())
						|| (list[0].getObjectId()==n2.getObjectId() && list[1].getObjectId()==n1.getObjectId()))
					return edge;
			}
		return null;
	}
