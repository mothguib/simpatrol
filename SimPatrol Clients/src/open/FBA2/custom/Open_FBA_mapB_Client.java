...
public class Open_FBA_mapB_Client  extends Client {
	double idleness_rate_for_path, idleness_rate_for_auction;
	double max_dist_entry;
	String env_file;
	

	public Open_FBA_mapB_Client(String remote_socket_address, int remote_socket_number,
	...
	public static void main(String[] args) {
		System.out.println("Open FBA Agents - map B!");
		...
		try {
			...
			Open_FBA_mapB_Client client = new Open_FBA_mapB_Client(
					remote_socket_address, remote_socket_number,
					environment_file_path, log_file_path, time_of_simulation,
					is_real_time_simulator, idleness_rate_for_path, idleness_rate_for_auction, max_dist_entry);
			client.start();
		} catch (Exception e) {
			System.out
					.println("Usage \"java open.FB2.custom.Open_FBA_mapB_Client \n"
							...
							+ "<rate of idleness in calculating heuristic destination> <rate of idleness in calculating nodes to trade>\n" +
									"<max distance to consider nodes, entry protocol>\n");
		}
	}
