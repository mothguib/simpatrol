...
/**
 * map A Open_FBA_Agent
 * 
 * Enter :  proximity
 * Quit : proximity
 * 
 * the 2 mechanisms use different distances
 *	I recommend using 1 * Av length of the map (=8) for entry, and 2 * Av length of the map (=16) for the exit
 *
 * works also for corridor
 *
 * 
 * @author pouletc
 *
 */
public class Open_FBA_mapA_Agent extends Open_FBA_Agent {

	protected double MAX_DIST_ENTRY;
	protected double MAX_DIST_EXIT;
	
	public Open_FBA_mapA_Agent(String id, double entering_time, double quitting_time, 
										int number_of_agents, LinkedList<String> nodes,
										double idleness_rate_for_path, double idleness_rate_for_auction, 
										double max_dist_entry, double max_dist_exit, String society_id) {
		...
		MAX_DIST_EXIT = max_dist_exit;
	}

	public Open_FBA_mapA_Agent(String id, int number_of_agents, LinkedList<String> nodes,
										double idleness_rate_for_path, double idleness_rate_for_auction,
										double max_dist_entry, double max_dist_exit) {
	...
	protected SpeechAct manage_Quit_Message(SpeechAct quitting_act) {
		...
		for(int i = 0; i < proposed_nodes.length; i++){
			if(this.get_minDistance_to_nodes(proposed_nodes[i], nodes) < this.MAX_DIST_EXIT){
