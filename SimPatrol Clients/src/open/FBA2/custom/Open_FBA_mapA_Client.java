...
public class Open_FBA_mapA_Client  extends Client {
	...
	public Open_FBA_mapA_Client(String remote_socket_address, int remote_socket_number,
	...
	public static void main(String[] args) {
		System.out.println("Open FBA Agents - map A!");
		...
		try {
			...
			Open_FBA_mapA_Client client = new Open_FBA_mapA_Client(
					remote_socket_address, remote_socket_number,
					environment_file_path, log_file_path, time_of_simulation,
					is_real_time_simulator, idleness_rate_for_path, idleness_rate_for_auction, max_dist_entry, max_dist_exit);
			client.start();
		} catch (Exception e) {
			System.out
					.println("Usage \"java open.FB2.custom.Open_FBA_mapA_Client \n"
							...
							+ "<rate of idleness in calculating heuristic destination> <rate of idleness in calculating nodes to trade>\n" +
									"<max distance to consider nodes, entry protocol><max distance to consider nodes, exit protocol>\n");
		}
	}
