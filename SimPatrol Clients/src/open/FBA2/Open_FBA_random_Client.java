...
public class Open_FBA_random_Client extends Client {
	
	double idleness_rate_for_path, idleness_rate_for_auction;
	String env_file;
	

	public Open_FBA_random_Client(String remote_socket_address, int remote_socket_number,
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				...
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Open_FBA_random_Agent(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, 
	...
	public static void main(String[] args) {
		...
		try {
			...
			Open_FBA_random_Client client = new Open_FBA_random_Client(
			...
			System.out
					.println("Usage \"java FBA.FlexibleBidder2Client\n"
