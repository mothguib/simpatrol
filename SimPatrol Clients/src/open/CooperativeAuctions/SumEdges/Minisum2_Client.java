...
public class Minisum2_Client  extends Client {
	double idleness_rate_for_path;
	String env_file;
	

	public Minisum2_Client(String remote_socket_address, int remote_socket_number,
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Minisum2_Agent(soc.agents[i].id, -1, soc.agents[i].quit_time, 
														nb_active_agents, mynodes, 
														idleness_rate_for_path, soc.agents[i].Society_to_join);
				...
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Minisum2_Agent(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, 
																nb_agents_in_system[i] + 1, new LinkedList<String>(), 
																idleness_rate_for_path, ordered_agents[i].Society_to_join);
	...
	public static void main(String[] args) {
		System.out.println("Open Minisum2 Social Agents ");
		...
		try {
			...
			Minisum2_Client client = new Minisum2_Client(
					remote_socket_address, remote_socket_number,
					environment_file_path, log_file_path, time_of_simulation,
					is_real_time_simulator, idleness_rate_for_path);
			client.start();
		} catch (Exception e) {
			System.out
					.println("Usage \"java open.Minisum2_Client \n"
