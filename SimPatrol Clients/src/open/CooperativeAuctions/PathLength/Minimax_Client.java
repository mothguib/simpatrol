...
public class Minimax_Client  extends Client {
	...
	public Minimax_Client(String remote_socket_address, int remote_socket_number,
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Minimax_Agent(soc.agents[i].id, -1, soc.agents[i].quit_time, 
				...
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Minimax_Agent(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, 
	...
	public static void main(String[] args) {
		...
		try {
			...
			Minimax_Client client = new Minimax_Client(
			...
			System.out
					.println("Usage \"java open.Minimax_Client \n"
