...
public class Minisum3_Client  extends Client {
	double idleness_rate_for_path;
	String env_file;
	

	public Minisum3_Client(String remote_socket_address, int remote_socket_number,
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Minisum3_Agent(soc.agents[i].id, -1, soc.agents[i].quit_time, 
				...
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Minisum3_Agent(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, 
	...
	public static void main(String[] args) {
		System.out.println("Open Minisum3 Social Agents ");
		...
		try {
			...
			Minisum3_Client client = new Minisum3_Client(
			...
			System.out
					.println("Usage \"java open.Minisum3_Client \n"
