...
public class Minimax3_Client  extends Client {
	...
	public Minimax3_Client(String remote_socket_address, int remote_socket_number,
			String environment_file_path, String log_file_path,
			double time_of_simulation, boolean is_real_time_simulator, 
			double idleness_rate_for_path, double distance)
		...
		for(SocietyImage soc : societies)
			if(!soc.id.equals("InactiveSociety"))
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Minimax3_Agent(soc.agents[i].id, -1, soc.agents[i].quit_time, 
				...
				for (int i = 0; i < soc.agents.length; i++) {
					...
					Agent agent = new Minimax3_Agent(ordered_agents[i].id, ordered_agents[i].enter_time, ordered_agents[i].quit_time, 
	...
	public static void main(String[] args) {
		System.out.println("Open Minimax3 Social Agents ");
		...
		try {
			...
			Minimax3_Client client = new Minimax3_Client(
			...
			System.out
					.println("Usage \"java open.Minimax3_Client \n"
