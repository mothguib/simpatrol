...
import open.FBA2.Open_FBA_random_Agent;
...
public class OpenHPCCClient extends Client {
	String env_file;
	 ...
	 */
	public OpenHPCCClient(String remote_socket_address,
		...
		OpenHPCCCoordinator.NO_COORD_HACK = !coord_hack;
	}
					...
					if(!soc.id.equals("InactiveSociety"))
						for (int i = 0; i < soc.agents.length; i++) {
							...
							if (agent_ids[corresponding_id].equals("coordinator"))
								agent = new OpenHPCCCoordinator();
							else
								agent = new OpenHPCCAgent(soc.agents[i].id, this.IS_REAL_TIME_SIMULATOR, -1, soc.agents[i].quit_time, soc.agents[i].Society_to_join);
						...
						for (int i = 0; i < soc.agents.length; i++) {
							...
							if (agent_ids[corresponding_id].equals("coordinator"))
								...
								agent = new OpenHPCCAgent(soc.agents[i].id, this.IS_REAL_TIME_SIMULATOR, soc.agents[i].enter_time, soc.agents[i].quit_time, soc.agents[i].Society_to_join);
	...
	public static void main(String[] args) {
		System.out.println("Open Heuristic cognitive coordinated agents!");
		...
		try {
			...
			OpenHPCCClient client = new OpenHPCCClient(
