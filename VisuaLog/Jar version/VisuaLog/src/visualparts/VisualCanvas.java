...
public class VisualCanvas extends JPanel {
	 ...
	 * Constructor
	 */
	public VisualCanvas(JFrame owner){
		super();
		visualog = owner;
		mytimer = new MyTimer();
	}
	...
	public void configure_canvas(){
		...
		if(((VisuaLog)visualog).getLogger().getType().contains("replay"))
			display_speed = true;
		else 
			display_speed = false;
		if(((VisuaLog)visualog).getLogger().getType().contains("turn"))
			display_turn = true;
		else 
			display_turn = false;
	...
	public void paint(Graphics g) {
		...
		if(display_speed){
			String speed_str = "Speed : " + String.valueOf(((ReplayLog)(((VisuaLog)visualog).getLogger())).getSpeed());
			g.drawChars(speed_str.toCharArray(), 0, speed_str.length(), 5, 30);
			if(!((ReplayLog)(((VisuaLog)visualog).getLogger())).isPlaying())
